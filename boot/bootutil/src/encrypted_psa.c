...
/* Fixme: This duplicates code from ecnrypted.c and depends on mbedtls */
static int
...
boot_decrypt_key(const uint8_t *buf, uint8_t *enckey)
{
    ...
    if (psa_ret != PSA_SUCCESS) {
        BOOT_LOG_ERR("Shared key import failed %d", psa_ret);
        return -1;
    }
    ...
    if (psa_cleanup_ret != PSA_SUCCESS) {
        BOOT_LOG_WRN("Shared key destruction failed %d", psa_cleanup_ret);
    }
    if (psa_ret != PSA_SUCCESS) {
     ...
     * respectivaley giving a derived key of the sum of the above.
    ...
    if (psa_ret != PSA_SUCCESS) {
        ...
        BOOT_LOG_ERR("MAC tag key import failed %d", psa_ret);
        return -1;
    }
    ...
    psa_cleanup_ret = psa_destroy_key(kid);
    if (psa_cleanup_ret != PSA_SUCCESS) {
        BOOT_LOG_WRN("MAC tag key destruction failed %d", psa_cleanup_ret);
    }
    if (psa_ret != PSA_SUCCESS) {
        memset(derived_key, 0, sizeof(derived_key));
        BOOT_LOG_ERR("MAC tag verification failed %d", psa_ret);
        return -1;
    }
    ...
    /* Import the AES prition of derived key, the first 16 bytes */
        ...
        const uint8_t *m, uint32_t mlen, size_t blk_off, uint8_t *c)
{
    ...
    /* Fixme: hardcoded counter  size, but it is hardcoded eveyrwhere */
        ...
        const uint8_t *c, uint32_t clen, size_t blk_off, uint8_t *m)
{
    ...
    /* Fixme: the init should already happen before calling the funciton, but
...
gone_after_setup:
    psa_ret = psa_cipher_abort(&psa_op);
    if (psa_ret != PSA_SUCCESS) {
        BOOT_LOG_WRN("PSA Decrypt abort failed %d", psa_ret);
        /* Intentionally not changing the ret */
    }
gone_with_key:
    psa_ret = psa_destroy_key(kid);
    if (psa_ret != PSA_SUCCESS) {
        BOOT_LOG_WRN("PSA Destroy key failed %d", psa_ret);
        /* Intentionally not changing the ret */
    }
gone:
    return ret;
