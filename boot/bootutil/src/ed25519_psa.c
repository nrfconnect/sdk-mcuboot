                          ...
                          const uint8_t public_key[32])
{
    ...
    if (status != PSA_SUCCESS) {
        BOOT_LOG_ERR("Failed to import key %d", status);
        return 0;
    }

    status = psa_verify_message(kid, PSA_ALG_PURE_EDDSA, message, message_len, signature,
    ...
    if (status != PSA_SUCCESS) {
        /* Just for logging */
        BOOT_LOG_ERR("Failed to destroy the key");
    }
