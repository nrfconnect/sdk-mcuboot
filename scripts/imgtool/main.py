...
def sign(ctx, key, public_key_format, align, version, pad_sig, header_size,
    ...
    if not aes_key:
       ...
       if encrypt:
           encrypt = None
           click.secho('Raw AES key overrides --key, there will be no encrypted key added to the image', err=True)
       if clear:
           click.secho('--clear overrides raw AES key, image will not be encrypted', err=True)
       if ctx.get_parameter_source('encrypt_keylen') != click.core.ParameterSource.DEFAULT:
           click.secho('Raw AES key len overrides --encrypt-keylen', err=True)
